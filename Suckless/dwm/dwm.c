...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
      ...
      /* The drw_rect code for small square indicators has been removed */
...
void updatebars(void) {
  ...
  for (m = mons; m; m = m->next) {
    ...
    XChangeProperty(dpy, m->barwin, netatom[NetWMWindowOpacity], XA_CARDINAL, 32,
        PropModeReplace, (unsigned char *)&baropacity, 1);
  }
}
...
updatesystray(void)
{
  ...
  if (!systray) {
    ...
    XChangeProperty(dpy, systray->win, netatom[NetWMWindowOpacity], XA_CARDINAL, 32,
        PropModeReplace, (unsigned char *)&baropacity, 1);
  }
